...
public class StudentController {
  ...
  //}
    ...
    public ResponseEntity<Student> getCourseWName(@RequestParam String name){
        int id = studentService.findIdByName(name);
        return ResponseEntity.ok().body(studentService.findById(id));
    }
    @DeleteMapping("/deletestudent/{name}")
    public ResponseEntity<List<Student>> deleteCourseWName(@PathVariable String name){
        int id = studentService.findIdByName(name);
        studentService.deleteFromDb(id);
        return ResponseEntity.ok().body(studentService.findAll());
    }
